...
class MediaDownloader:
    ...
    def _convert_heic_to_jpeg(self, file_path: str) -> str:
        if not HEIF_SUPPORT:
            logger.warning("pillow-heif 未安装，无法转换 HEIC 格式")
            return file_path
        ...
        try:
            ...
            os.remove(file_path)
            logger.debug(f"HEIC 转换成功: {file_path} -> {jpeg_path}")
            return jpeg_path
        except Exception as e:
            logger.warning(f"HEIC 转换失败: {e}")
            return file_path
        ...
        async with self._semaphore:
            for attempt in range(max_retries + 1):
                try:
                    ...
                    temp_path = os.path.join(save_dir, f"temp_{int(time.time())}_{attempt}.tmp")
        ...
        for url in urls:
            if url:
                ...
                if result:
                    return result
        if urls:
            logger.warning(f"所有备用URL下载失败: {urls[0][:50]}...")
        return None
    ...
    async def download_items_media(self, items: List[Dict], id_field: str, 
        ...
        year_month = timestamp_to_year_month(video_timestamp) if video_timestamp else None
        
        for item in tqdm(items, desc="下载媒体", unit="个"):
            item_id = item.get(id_field)
            if not item_id:
                continue
            
            item_year_month = year_month or timestamp_to_year_month(item.get(time_field))
    ...
    async def download_avatars_stickers(self, items: List[Dict], id_field: str,
        ...
        for coro in tqdm(asyncio.as_completed(download_tasks), total=len(download_tasks), desc="下载头像/表情", unit="个"):
